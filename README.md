...
현재 프로젝트는 로컬 개발 환경에서 동작을 검증하는 MVP 단계입니다.

Firebase Hosting은 React/Vite 빌드 결과물인 정적 파일 배포에는 적합하지만, Express `server.js` 백엔드 서버를 직접 실행할 수는 없습니다. 따라서 프론트엔드는 Firebase Hosting을 향후 배포 후보로 둘 수 있고, 백엔드는 Render Web Service 또는 Cloud Run 같은 별도 Node.js 실행 환경을 고려하는 것이 현실적입니다.

| 영역 | 배포 방향 |
| --- | --- |
| Frontend | Firebase Hosting 후보 |
| Backend | Render Web Service 우선 고려, Cloud Run/Firebase Functions 검토 가능 |
...
현재 단계에서는 로컬 MVP를 안정적으로 유지하고, 이후 Render Web Service와 Firebase Hosting을 활용한 배포 확장을 고려합니다.
