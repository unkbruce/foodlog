...
| Lint | ESLint |
| Deployment | Frontend Firebase Hosting, Backend Render Web Service |
...
배포 빌드에서는 `VITE_API_BASE_URL`을 통해 Render 백엔드 주소를 사용합니다. 예시는 [.env.example](.env.example)에서 확인할 수 있습니다.

```env
VITE_API_BASE_URL=https://foodlog-api-jz7l.onrender.com
```

로컬에서 Express 서버와 Vite 개발 서버를 함께 실행하는 경우 `.env` 없이도 Vite proxy를 통해 백엔드 서버로 요청할 수 있습니다. 실제 환경 변수 파일인 `.env`, `.env.local`, `.env.production`은 Git에 올리지 않고, `.env.example`만 예시로 관리합니다.
...
현재 프론트엔드는 Firebase Hosting에, 백엔드는 Render Web Service에 배포되어 있습니다. 프론트엔드에서 `VITE_API_BASE_URL`로 설정된 Render 백엔드 API를 호출하는 구조입니다.

| 영역 | 배포 상태 |
| --- | --- |
| Frontend | Firebase Hosting 배포 완료 |
| Backend | Render Web Service 배포 완료 |
| Database | 현재 Express 서버의 인메모리 배열 사용 |
| 향후 개선 | Firestore 또는 DB 연동을 통한 데이터 영속성 개선 |

인메모리 배열 기반 데이터는 서버 재시작 시 초기화될 수 있습니다. 향후 Firestore 또는 별도 데이터베이스를 연동해 데이터 영속성을 개선할 수 있습니다.

## 배포 주소

### Frontend

* Firebase Hosting: `https://planning-with-ai-f6aa8.web.app`

### Backend API

* Render Web Service: `https://foodlog-api-jz7l.onrender.com`
...
Firebase Hosting에 배포된 프론트엔드에서 Render 백엔드 API를 호출할 수 있도록 Express 서버에 CORS 허용 origin을 설정했습니다.
