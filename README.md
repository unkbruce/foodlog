...
| Lint | ESLint |
| Deployment | Render 배포 고려, Firebase Hosting은 향후 프론트 배포 후보 |
...
현재 프로젝트는 로컬 개발 환경에서 동작을 검증하는 MVP 단계입니다. 이번 배포 준비 범위는 Express 백엔드 API를 Render Web Service로 먼저 배포하는 것입니다.

Firebase Hosting은 React/Vite 빌드 결과물인 정적 파일 배포에는 적합하지만, Express `server.js` 백엔드 서버를 직접 실행할 수는 없습니다. 프론트엔드 Firebase Hosting 배포와 API base URL 변경은 Render 백엔드 URL이 확정된 다음 단계에서 진행합니다.

| 영역 | 배포 방향 |
| --- | --- |
| Frontend | 아직 로컬/Vite 개발 서버 기준, 향후 Firebase Hosting 후보 |
| Backend | Render Web Service 우선 배포 |
| Database | 현재는 인메모리 배열, 향후 Firestore 또는 별도 DB 연동 가능 |

Render Web Service 설정값은 다음과 같습니다.
...
Render 배포 후에는 발급된 서비스 URL 기준으로 다음 API를 확인할 수 있습니다.

* `GET https://<render-service-url>/foods`
* `GET https://<render-service-url>/foods/:id`
* `POST https://<render-service-url>/foods`
* `PUT https://<render-service-url>/foods/:id`
* `DELETE https://<render-service-url>/foods/:id`
* `GET https://<render-service-url>/feedback`

현재 프론트엔드 API 호출 경로는 아직 변경하지 않았습니다. Render 배포 URL이 나온 뒤 `VITE_API_BASE_URL` 또는 배포 환경에 맞는 API 경로를 별도로 설정합니다.
